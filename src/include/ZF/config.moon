...
class CONFIG
    ...
    mkdir: (dir) =>
        ...
        unless @file_exist(dir, true)
            os.execute "mkdir #{dir}"

    -- deletes one or more folders
    rmdir: (dir) =>
        assert dir, "expected dir"
        if @file_exist(dir, true)
    ...
    load: (GUI, macro_name) =>
        dir = "zeref-cfg\\#{macro_name\lower!\gsub("%s", "_")}.cfg"
        read, len = @read dir
    ...
    save: (GUI, elements, macro_name, macro_version) =>
        ...
        unless @file_exist "zeref-cfg", true
            @mkdir "zeref-cfg"
        save = "zeref-cfg\\#{macro_name\lower!\gsub("%s", "_")}.cfg"
        file = io.open save, "w"
        file\write writing
        file\close!
        return
        ...
        switch macro_name
            when "Envelope Distort"
                ->
                    ...
                    }
                    {
                        ...
                        {class: "intedit", name: "tol", hint: hints[3], x: 1, y: 1, width: 2, min: 1, value: 2}
                        {class: "label", label: "Type:  ", x: 1, y: 2}
                        {class: "dropdown", name: "tpc", items: items[1], hint: hints[4], x: 1, y: 3, width: 2, value: items[1][2]}
                        {class: "checkbox", name: "rfl", label: "Remove selected layers?", x: 0, y: 4, value: true}
                        {class: "checkbox", name: "srt", label: "Sort to clockwise?\t\t\t ", x: 1, y: 4, value: true}
                    }
            ...
            when "Interpolate Tags"
                (allTags) ->
                    ...
                    table.sort allTags
                    tags = [TABLE(allTags)\slice(k, k + 7) for k = 1, #allTags, 8]
                    for k = 1, #tags
                        for j = 1, #tags[k]
                            gui[#gui + 1] = {
                                class: "checkbox"
                    ...
                    gui[#gui + 1] = {class: "checkbox", label: "Ignore Text", name: "igt", x: 0, y: 9, value: false}
                    gui[#gui + 1] = {class: "label", label: "::Acceleration::", x: 0, y: 10}
                    gui[#gui + 1] = {class: "floatedit", name: "acc", hint: "Relative interpolation acceleration.", min: 0, x: 0, y: 11, value: 1}
                    return gui
            when "Make Image"
                (typer = "tracer") ->
                    drops = {
